...
execute as @e[tag=platform_marker,scores={SML.isInit=1},distance=..10] at @s positioned ~-1.5 ~-1 ~-1.5 run function mechanic:redstone/bulb/mark_entitys_platform
execute as @e[tag=lift_marker,scores={SML.isInit=1},distance=..10] at @s positioned ~-0.5 ~ ~-0.5 run function mechanic:redstone/bulb/mark_entitys_base

# link if valid
execute positioned ~0.5 ~0.5 ~0.5 as @e[tag=bulb_marker,scores={SML.onLift=1..},distance=..0.2] at @s run function mechanic:redstone/common/link_to_lift
